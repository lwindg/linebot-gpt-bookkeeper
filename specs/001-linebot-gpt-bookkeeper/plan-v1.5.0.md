...
## v1.5.0 新功能：支援單一訊息多筆支出

使用者可能在一條訊息中描述多筆支出，例如：
- 「早餐80元現金，午餐150元刷卡」
- 「昨天買了咖啡50；點心120用狗卡」
- 「早餐三明治35現金，飲料30現金」
...
1. **判斷意圖**：
   - 單筆記帳：僅包含一個品項的支出
   - 多筆記帳：包含多個獨立品項的支出（用逗號、分號、頓號、換行分隔）
   - 一般對話：非記帳相關訊息

2. **多筆記帳處理**：
   - 識別每一筆獨立的支出項目
   - 為每一筆提取：品項、金額、付款方式
   - **重要**：每一筆都必須包含完整資訊（品項、金額、付款方式）
   - 若任一筆不完整，回傳 error intent

3. **區分模糊情況**：
   - 「買了三明治和咖啡80元現金」 → **無法確定**是一筆還是兩筆，回傳 error
   - 「早餐80元現金，午餐150元刷卡」 → **明確兩筆**，分別處理
...
```
┌─────────────────────────────────────────────────────────────┐
│ 1. 使用者在 LINE 發送：「早餐80元現金，午餐150元刷卡」          │
└─────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────┐
...
│    POST #1: { 品項:"早餐", 金額:80, 交易ID:20251114-143052 }│
│    POST #2: { 品項:"午餐", 金額:150, 交易ID:20251114-143052}│
└─────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────┐
│ 8. 構建確認訊息（列出兩筆）                                   │
...
```python
# 多筆支出測試
def test_multi_expense_two_items():
    """測試識別兩筆支出（逗號分隔）"""
...
def test_multi_expense_incomplete():
    """測試多筆支出但第二筆資訊不完整"""
    result = process_multi_expense("早餐80元現金，午餐買了便當")
    assert result.intent == "error"
    assert "第二筆" in result.error_message

def test_multi_expense_ambiguous():
    """測試模糊情況（無法區分一筆還是兩筆）"""
    result = process_multi_expense("買了三明治和咖啡80元現金")
    assert result.intent == "error"
    assert "無法確定" in result.error_message

def test_multi_expense_shared_transaction_id():
    """測試多筆支出共用交易ID"""
...
#### 多筆文字支出
- [ ] 「早餐80元現金，午餐150元刷卡」 → 記錄兩筆，共用交易ID
- [ ] 「咖啡50；點心120用狗卡」（分號分隔） → 正確識別
- [ ] 「早餐三明治35，飲料30，現金」 → 兩筆都用現金
- [ ] 「早餐80元現金，午餐買了便當」（第二筆不完整） → 錯誤提示
- [ ] 「買了三明治和咖啡80元現金」（模糊） → 錯誤提示
...
- ✅ 「早餐80元現金，午餐150元刷卡」 → 記錄兩筆，共用交易ID
- ✅ 兩筆支出分別發送 webhook 到 Make.com
- ✅ 確認訊息列出所有項目
- ✅ 不完整資訊提示清楚（如「第二筆缺少金額」）
- ✅ 模糊情況提示清楚（如「無法確定是一筆還是多筆」）
...
**決策**：單一訊息的所有支出共用同一個時間戳記交易ID

- **理由**：
  - 符合「同一次記帳行為」的概念
  - 方便使用者追蹤「這次記了哪幾筆」
  - Make.com 可透過相同 ID 關聯多筆支出
