...
- 若「原幣金額唯一命中」但匯率 sanity check 未通過，保留為 weak candidate：
  - 在沒有 strong candidate 的前提下，可自動匹配為 `matched`
  - 若 weak candidate 有多筆，維持 `unmatched`（避免誤配）
...
- 批次加總需包含同批次所有品項（含消費稅等拆項），不能只看單筆品項。
- 候選帳目查詢必須完整分頁（不可只取前 100 筆），避免明細多時遺漏整個批次。
