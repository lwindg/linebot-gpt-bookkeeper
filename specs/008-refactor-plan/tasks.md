...
- [x] T001 建立權威 JSON Schema（擴充 `app/schemas.py`）
  - 新增 `AUTHORITATIVE_ENVELOPE_SCHEMA`
  - 定義 version 相容策略（v1 → v1.1）
- [x] T002 建立 AI Enrichment Schema（擴充 `app/schemas.py`）
  - 新增 `ENRICHMENT_RESPONSE_SCHEMA`
- [x] T003 抽出分類清單到 `app/config/classifications.yaml`
  - 從 `prompts.py:CLASSIFICATION_RULES` 抽取 50+ 分類
- [x] T004 抽出付款方式到 `app/config/payment_methods.yaml`
  - 從 `payment_resolver.py:_PAYMENT_ALIASES` 抽取 mapping
- [x] T005a 更新 `app/category_resolver.py` 讀取 YAML
  - 移除對 `prompts.CLASSIFICATION_RULES` 的依賴
- [x] T005b 更新 `app/payment_resolver.py` 讀取 YAML
...
- [x] T016 新增精簡 AI enrichment prompt `app/prompts_enrichment.py`
  - 目標 <200 行（vs 現有 764 行）
  - 移除抽取規則，保留分類/專案/必要性規則
- [x] T017 製作 Parser → AI payload 轉換 `app/enrichment/prepare_payload.py`
  - `AuthoritativeEnvelope` → GPT messages
- [x] T018 實作 AI enrichment 呼叫 `app/enrichment/client.py`
...
- [x] T019 建立 enrichment validator `app/enrichment/validator.py`
  - 分類必須在 YAML 清單內
  - 必要性必須在 enum 內
- [x] T020 實作 fallback 策略 `app/enrichment/fallbacks.py`
  - 分類不合法：回退上層（如 `餐飲類` → `家庭/餐飲`）
  - 必要性缺失：預設 `必要日常支出`
  - 專案缺失：依分類推斷或預設 `日常`
- [x] T021 實作 AI 重試邏輯 `app/enrichment/retry.py`
  - 最多重試 1 次（可配置）
...
- [x] T022 實作 shadow mode 比對器 `app/pipeline/shadow.py`
  - 同時執行舊路徑與新路徑
- [x] T023 記錄比對指標（log/metrics）
...
- [x] T024 新增主流程 `app/pipeline/main.py`
  - Parser → AI Enrichment → Validator → 輸出
- [x] T025 整合至 `app/gpt_processor.py`
  - 新增 `process_message_v2()` 入口
- [x] T026 保留舊路徑為 fallback（config `USE_PARSER_FIRST`）
- [x] T027 明確混合句型策略
  - 現金流僅支援單筆
  - 一般支出可多項目
  - 混合 → 回錯
- [x] T028 新增 error 分類記錄
