...
CURRENCY_DETECTION = """## 幣別識別規則（v003-multi-currency 新增）
...
**預設規則**：
- 當訊息中**無幣別關鍵字**時，設定 `原幣別 = "TWD"`（新台幣）
- 系統會自動查詢匯率並換算成新台幣

**範例**：
- ✅ 「WSJ 4.99美元 大戶」→ 原幣別: `"USD"`, 原幣金額: 4.99
- ✅ 「Norrona jacket 290.97歐元 灰狗卡」→ 原幣別: `"EUR"`, 原幣金額: 290.97
- ✅ 「OpenAI API Key 10美金 大戶」→ 原幣別: `"USD"`, 原幣金額: 10
- ✅ 「便當 80 現金」→ 原幣別: `"TWD"`, 原幣金額: 80（無幣別關鍵字，預設為 TWD）

**重要**：
1. 幣別關鍵字通常位於金額之後、付款方式之前
2. 金額可能包含小數點（如 4.99、290.97）
3. 同一訊息可能包含多筆不同幣別的消費（請分別識別）
"""
