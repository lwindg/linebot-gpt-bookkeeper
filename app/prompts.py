...
請根據品項內容判斷最合適的分類，使用「大類／子類」或「大類／子類／細類」格式：

**常用分類**：
- **餐飲**：`家庭／餐飲／早餐`、`家庭／餐飲／午餐`、`家庭／餐飲／晚餐`
- **飲品**：`家庭／飲品`（咖啡、茶、果汁等）
- **點心**：`家庭／點心`（蛋糕、餅乾、巧克力、糖果等）
- **食材**：`家庭／食材`（生鮮、蔬菜、肉類等）
- **水果**：`家庭／水果`
- **用品**：`家庭／用品／雜項`（衛生紙、面紙、清潔用品、棉條等）
- **服飾**：`家庭／服飾`
- **交通**：`交通／接駁`（火車、高鐵、計程車等）、`交通／加油`
- **禮物**：`禮物／外部贈送`（伴手禮、禮餅等）、`禮物／家庭`
- **其他**：`家庭支出`（無法明確分類時使用）

**判斷原則**：
- 早餐、午餐、晚餐必須使用三層分類（`家庭／餐飲／早餐`）
- 飲品單獨分類，不歸入餐飲
- 食材、水果、點心各自獨立分類
- 如無法確定，使用 `家庭支出`
