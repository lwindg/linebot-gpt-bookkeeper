...
1. **有幣別關鍵字** → 設定對應的 ISO 代碼
   ...
   - ✅ 「Netflix 15.99USD 信用卡」→ `原幣別: "USD"`, `原幣金額: 15.99`
   - ✅ 「飯店 290.97歐元 信用卡」→ `原幣別: "EUR"`, `原幣金額: 290.97`
   - ✅ 「OpenAI 10美金 大戶」→ `原幣別: "USD"`, `原幣金額: 10`

2. **無幣別關鍵字** → 預設為 TWD
   - ✅ 「便當 80 現金」→ `原幣別: "TWD"`, `原幣金額: 80`
   - ✅ 「午餐 120 現金」→ `原幣別: "TWD"`, `原幣金額: 120`

3. **幣別關鍵字位置**：通常在金額之後、付款方式之前
   - 格式：`品項 + 金額 + 幣別 + 付款方式`
   - 範例：「WSJ 4.99美元 大戶」「Netflix 15.99USD 信用卡」

4. **金額格式**：可能包含小數點（如 4.99、290.97）

### ⚠️ 常見錯誤（務必避免）

❌ **錯誤**：「WSJ 4.99美元 大戶」→ 原幣別: "TWD"（忽略了「美元」關鍵字）
✅ **正確**：「WSJ 4.99美元 大戶」→ 原幣別: "USD"

❌ **錯誤**：「Netflix 15.99USD 信用卡」→ 原幣別: "TWD"（忽略了「USD」關鍵字）
✅ **正確**：「Netflix 15.99USD 信用卡」→ 原幣別: "USD"
"""
...
**禁止輸出暱稱**：例如輸入「灰狗」也必須輸出 `FlyGo 信用卡`，不可輸出「灰狗卡」。
"""
...
- 「同事先墊午餐費150元」→ `payment_method="NA"`, items: `{品項:"午餐費", 原幣金額:150, 代墊狀態:"需支付", 收款支付對象:"同事"}`
...
- 「咖啡50元，蛋糕120元，狗卡」（多項目，付款方式在最後且單獨出現）
...
**外幣識別**：請嚴格遵守上方 `{CURRENCY_DETECTION}`（有幣別關鍵字必須轉成對應 ISO 代碼）。
...
1. **判斷意圖**（僅支援以下四種 intent）：
   ...
   - **update_last_entry**：修改上一筆記帳（v1.9.1 擴充）
     - 使用者想修改最近一次記帳的某個欄位（品項、分類、專案、金額）
...
2. **單項目 vs 多項目判斷規則**：
   ...
   **識別步驟**：
   ...
   - **單項目範例**：
     ...
     - ✅ 「豬肉1430現金」→ 1個項目（豬肉+1430元），付款方式：現金（品項+金額+付款，完全連寫）
     - ✅ 「雞肉$770現金」→ 1個項目（雞肉+770元），付款方式：現金（品項+$+金額+付款，無空格）
     - ✅ 「菇 240現金」→ 1個項目（菇+240元），付款方式：現金（品項+空格+金額付款連寫）
     - ✅ 「蔬果菇類$240現金」→ 1個項目（蔬果菇類+240元），付款方式：現金（長品項+$+金額+付款）
     - ✅ 「菇類240現金」→ 1個項目（菇類+240元），付款方式：現金（品項+金額+付款，完全連寫）
     - ✅ 「食材$500狗卡」→ 1個項目（食材+500元），付款方式：台新狗卡（品項+$+金額+付款暱稱）
     - ✅ 「魚350狗卡」→ 1個項目（魚+350元），付款方式：台新狗卡（品項+金額+付款暱稱）
...
4. **記帳處理**：
   ...
   - **金額識別規則**：
     ...
     - 金額通常在品項之後、付款方式之前
     - **連寫格式**：品項、金額、付款方式可以無空格連寫（例：「魚$395現金」、「豬肉1430現金」）
     - **$ 符號識別**：當出現 $ 時，$ 後的數字為金額（例：「魚$395」中的「395」）
     - **邊界判斷**：在連寫格式中，通過數字和付款方式關鍵字判斷金額邊界（例：「豬肉1430現金」→ 金額=1430，付款方式=現金）
   - **品項提取規則**（品項 = 實際購買的物品）：
...
2. **付款方式在最外層**：所有項目共用同一付款方式
   - 若訊息最後只出現付款方式暱稱（例如「…, 狗卡」），該段**不是項目**，必須填入 `payment_method`
3. **早餐/午餐/晚餐必須使用三層分類**：家庭/餐飲/早餐、家庭/餐飲/午餐、家庭/餐飲/晚餐
4. **代墊狀態預設為「無」**，收款支付對象預設為空字串
5. **多項目時每個項目都必須有金額**：
   - ✅ 「早餐80元，午餐150元，現金」→ 2 個項目
   - ❌ 「早餐，午餐150元，現金」→ 回傳 `error`（第一項缺少金額）
6. **品項包含人名時的判斷**：
   - 無代墊關鍵字 → 人名是品項的一部分
...
1. **必要欄位檢查**：品項、金額、付款方式三者缺一不可，缺少任一項必須回傳 error
   - 需支付例外：若判斷為「需支付」（別人先墊/先付），且訊息未提供付款方式，`payment_method` 必須填 `"NA"`
