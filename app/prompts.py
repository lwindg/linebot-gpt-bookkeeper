...
**注意**：`date` 欄位為可選欄位，若訊息中包含日期資訊（如「11/12 早餐80元」），則提取到此欄位；若無日期資訊，可省略或設為 null。
...
**注意**：單項目也使用 `"intent": "multi_bookkeeping"`，items 陣列只有一個元素。早餐、午餐、晚餐**必須**使用三層分類結構。代墊狀態預設為「無」，收款支付對象為空字串。
...
**重要**：訊息「妹9-10月諮商費18900合庫」中沒有代墊關鍵字（代、幫、墊等），所以「妹」是品項的一部分，不是代墊對象。品項應完整保留為「妹9-10月諮商費」。
