...
**範例分析**：
1. 「11/12 午餐$120現金」
   - ✅ 有日期：「11/12」→ 解析為「2025-11-12」
   - ❌ 錯誤：當成今天（2025-11-13）

2. 「11/12 14:30 午餐$120現金」
   - ✅ 有日期：「11/12」→「2025-11-12」
   - ✅ 有時間：「14:30」
   - ❌ 錯誤：忽略日期和時間
...
**檢查步驟**：
1. 看訊息開頭有沒有數字（如「11/12」）
2. 如果有，提取為日期並補上年份（2025）
3. 如果沒有，才使用今天
...
2. **記帳處理**（若為記帳意圖）：
   - **提取資訊**（從自然語句中靈活提取）：
     - **日期和時間**：
       - 日期格式：「11/12」→「2025-11-12」、「11月12日」→「2025-11-12」、「今天」→今天日期
       - 時間格式：「14:30」（僅在明確提供時才提取）
       - ⚠️ **請參考頂部的「關鍵提醒：日期解析」區塊**
     - **品項**：主要消費類別
