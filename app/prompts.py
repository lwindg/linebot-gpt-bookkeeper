...
6. **現金流意圖（cashflow_intents）**：
   - 關鍵字（優先序）：繳卡費 > 轉帳 > 提款 > 收入
   - `現金流意圖` 值僅允許：`withdrawal` / `transfer` / `income` / `card_payment`
   - `交易類型` 使用中文：`提款` / `轉帳` / `收入` / `支出` / `折扣`
   - 對他人轉帳 → `交易類型` 為「支出」
   - 帳戶間轉帳 → 交易類型為「轉帳」與「收入」
   - 提款 → 交易類型為「提款」與「收入」
   - 繳卡費視為轉帳（帳戶 → 信用卡）
...
- `fields_to_update`: 要更新的欄位物件（物件中只包含需要更新的欄位）
  ...
  - 範例：
    ...
    - {{"付款方式": "富邦 Costco"}} - 只修改付款方式（必須使用標準名稱）
    - {{"明細說明": "補登 Costco"}} - 只修改明細說明
    - {{"必要性": "必要日常支出"}} - 只修改必要性（必須使用標準值）
