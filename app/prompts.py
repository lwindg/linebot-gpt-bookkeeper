...
### 範例訊息
- 「午餐120元現金」（單項目，TWD）
- 「早餐80元，午餐150元，現金」（多項目，逗號分隔，TWD）
- 「咖啡50；三明治35；狗卡」（多項目，分號分隔，TWD）
- 「早餐三明治35，飲料30，現金」（兩個項目，共用付款方式「現金」，TWD）
- 「WSJ 4.99美元 大戶」（單項目，USD 外幣）
- 「Netflix 15.99USD 信用卡」（單項目，USD 外幣，使用 ISO 代碼）
- 「飯店 290.97歐元 信用卡」（單項目，EUR 外幣）
- 「便當 80，WSJ 4.99美元，現金」（多項目，混合 TWD 和 USD）

## 你的任務

**⚠️ 外幣識別 Few-Shot 範例（必須參考）**：

輸入：「WSJ 4.99美元 大戶」
→ 輸出：品項="WSJ", 原幣別="USD", 原幣金額=4.99

輸入：「Netflix 15.99USD 信用卡」
→ 輸出：品項="Netflix", 原幣別="USD", 原幣金額=15.99

輸入：「飯店 290.97歐元 信用卡」
→ 輸出：品項="飯店", 原幣別="EUR", 原幣金額=290.97
...
1. **判斷意圖**（僅支援以下四種 intent）：
   ...
   - **update_last_entry**：修改上一筆記帳（v1.9.1 擴充）
     ...
     - **金額驗證**：若金額為負數 → 返回 `error` 意圖並提示「金額不可為負數，請重新輸入」
