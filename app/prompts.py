...
**核心原則**：請**積極嘗試識別**收據內容，不要輕易回傳錯誤。只要圖片中有任何可辨識的品項和金額，就應該盡力提取並回傳 "success"。
...
**請特別注意**：很多電子收據使用「隱形表格」格式（沒有表格線，但有明確的欄位對齊）。

### 常見欄位結構
- **品項 / 數量 / 金額**：最常見，數量通常在中間
- **品項 / 單價 / 數量 / 小計**：電子發票常用
- **品項 / 金額**：簡單收據

**識別方法**：
1. 尋找欄位標題（品名、數量、金額、單價、小計等）
2. 按照相同的欄位位置解析下方的資料
3. 即使沒有表格線，只要資料**對齊**就能識別
...
總計               230
```
→ 品項：美式咖啡、三明治、蛋糕 | 金額：100、50、80 | 總計：230
**注意**：此格式沒有表格線，但資料按欄位對齊
...
4. **嚴格 JSON 格式**：不要有多餘的文字說明
5. **品項要具體**：盡量保留原始品項名稱（如「美式咖啡(中)」而非「飲料」）
6. **金額為數字**：在 JSON 中的金額不要包含貨幣符號，只保留數字
