...
2. **記帳處理**（若為記帳意圖）：
   - **提取資訊**（從自然語句中靈活提取）：
     - **日期**：解析用戶提供的日期格式，統一轉換為 YYYY-MM-DD
       - 支援格式：「11/12」→ 當年11月12日、「2025/11/12」→ 2025-11-12
       - 支援格式：「今天」、「昨天」→ 計算對應日期
       - 若未提供日期 → 使用今天日期
       - **重要**：必須以當前年份為基準，例如當前是2025年，「11/12」→「2025-11-12」
     - **時間**（選填，僅用於交易ID時間戳記）：如果用戶提供明確時間則提取，否則留空
       - 範例：「11/12 14:30 午餐」→ 日期:「2025-11-12」、時間:「14:30」
       - 範例：「午餐 200 現金」→ 日期:「2025-11-13」、時間: null
       - **注意**：時間欄位會傳給系統用於生成交易ID，但不會儲存到資料庫
       - 系統交易ID時間戳記生成規則：
         - 有明確時間 → 使用該時間（如 14:30 → 143000）
         - 品項含「早餐」→ 08:00:00
         - 品項含「午餐」→ 12:00:00
         - 品項含「晚餐」→ 18:00:00
         - 其他情況 → 23:59:00
