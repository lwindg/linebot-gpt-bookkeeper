...
2. **記帳處理**（若為記帳意圖）：
   - **提取資訊**（從自然語句中靈活提取）：
     - **日期**：解析用戶提供的日期格式，統一轉換為 YYYY-MM-DD
       - **支援格式範例**：
         - 「11月11日」→「2025-11-11」（中文格式，補當前年份）
         - 「11/12」→「2025-11-12」（月/日格式，補當前年份）
         - 「2025/11/12」→「2025-11-12」（完整年/月/日）
         - 「今天」→ 當天日期
         - 「昨天」→ 前一天日期
       - **解析步驟**：
         1. **仔細檢查訊息開頭**是否有日期（如「11/12」、「11月11日」）
         2. 識別日期格式（中文、斜線、相對日期）
         3. 提取月份和日期數字
         4. 如果沒有年份，**必須補上當前年份（2025年）**
         5. 轉換為標準格式 YYYY-MM-DD
       - **重要範例**：
         - 輸入「11/12 午餐$120現金」→ 日期:「2025-11-12」（**不是今天**）
         - 輸入「11/12 14:30 午餐$120現金」→ 日期:「2025-11-12」、時間:「14:30」
         - 輸入「午餐$120現金」（無日期）→ 日期:「2025-11-13」（今天）
       - **重要**：若用戶未提供日期，預設為今天
     - **時間**（選填）：如果用戶提供明確時間則提取，否則不要猜測
       - 範例：「11/12 14:30 午餐」→ 日期:「2025-11-12」、時間:「14:30」
       - 範例：「午餐 200 現金」→ 日期:「2025-11-13」、時間: 不填寫
     - **品項**：主要消費類別
