...
_UPDATE_FIELD_PATTERN = re.compile(
    r"(?:修改|更改|改|更新)\s*(?P<field>品項|分類|專案|原幣金額|金額|原幣別|幣別|匯率|付款方式|明細說明|明細|必要性)\s*(?:為|成)?\s*(?P<value>.+)$"
)
_UPDATE_FIELD_REVERSED_PATTERN = re.compile(
    r"(?P<field>品項|分類|專案|原幣金額|金額|原幣別|幣別|匯率|付款方式|明細說明|明細|必要性)\s*(?:修改|更改|改|更新)\s*(?:為|成)?\s*(?P<value>.+)$"
)
