...
1. **判斷圖片類型**：
   ...
   - **特別注意**：日文收據（包含「領収書」、「レシート」字樣）是**有效收據**，必須進行識別。
   - 若明顯非收據（如風景照、人物照、一般物品照片），回傳 "not_receipt"。

2. **提取資訊**（若為收據）：
   - **品項名稱**：購買的商品或服務名稱（例如：美式咖啡、三明治、拿鐵、午餐、便當等）。
     - 品項可能以各種格式出現：「咖啡」、「Coffee」、「美式咖啡（中杯）」等。
     - 如果有多個商品，全部列出。
     - **翻譯規則（極重要）**：若原始品項名稱非繁體中文（如日文、英文等），你**必須**將其翻譯為**繁體中文**填入「品項」欄位。例如「おにぎり」翻譯為「飯糰」。
     - **原文保留**：請將原始的完整品項名稱（未翻譯前的文字）填入「原文」欄位。
...
3. **幣別與語言判斷**：
   - 支援 TWD、USD、EUR、JPY、GBP、AUD、CAD、CNY
   - **日文收據**：包含「領収書」、「レシート」等字樣，品項通常為日文，幣別通常為 JPY。
   - 台灣收據常用 $ 符號代表台幣，不是美金。
   - 若無法判斷幣別，預設為 TWD。

4. **品質檢查**（請寬容判斷，不要輕易回傳錯誤）：
   - 只要圖片包含文字、數字且看起來像消費紀錄，就應回傳 "success"。
   - 圖片模糊到**完全無法辨識文字** → 回傳 "unclear"。
   - **真正缺少**品項或金額（圖片中確實沒有這些資訊） → 回傳 "incomplete"。
   - **重要**：日文、英文等外國收據是**有效收據**，請務必處理並進行翻譯。
