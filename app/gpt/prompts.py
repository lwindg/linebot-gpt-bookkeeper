...
| 日圓、日幣、日元、JPY、jpy、¥、円 | `"JPY"` | 「1000日圓」「500JPY」「¥500」「1000円」「2000日元」 |
...
|-------------|-------------|
| 現金、cash | `現金` |
| Line、Line Pay、Line Bank | `Line Pay` |
| 合庫 | `合庫` |
| 狗卡、狗狗卡、GoGo、狗 | `台新狗卡` |
...
- `家庭/酒品` - 啤酒、紅酒、威士忌等酒精飲品
- `家庭/休閒與運動` - 玩具、運動用品、樂器、模型、露營器材
- `家庭/家電` - 大型電器、共用電腦維修
- `家庭/用品` - 耐用品（收納、五金、餐具、毛巾）
- `家庭/身體護理` - 身體消耗品（牙膏、洗沐品、護唇膏、防曬）
- `家庭/居家清潔` - 環境消耗品（衛生紙、清潔劑、洗碗精）
- `家庭/服飾` - 家人共用、兒童衣物、家庭採購服飾
- `家庭/公用事業` - 水費、電費、瓦斯費、大樓管理費（不含稅金）
- `家庭/通訊/網路費` - 家中寬頻、光纖、Wi-Fi 等上網費
- `家庭/通訊/手機費` - 手機月租、行動上網、子女手機費
- `家庭/娛樂` - 電影票、遊戲軟體、數位內容訂閱
- `家庭/圖書` - 書籍、雜誌、教材
- `家庭/津貼` - 小孩零用錢、給父母的錢、紅包補貼
- `家庭/雜項` - 無法分類小額支出、手機換電池等

**個人類別**：
- `個人/餐飲` - 公司聚餐、單人外食
- `個人/服飾` - 自己的衣物、鞋子、飾品
- `個人/美容護理` - 專屬美容、彩妝、高階個人保養
- `個人/用品` - 個人專屬耐用品
- `個人/公益` - 個人公益或捐款
- `個人/車資` - 個人專用車資、Uber、捷運、日常共乘費
- `個人/通訊` - 個人手機月租、行動上網費
- `個人/交際` - 建立關係、請客開支、商業社交餐

**行程類別**：
- `行程/食材` - 登山糧食、行程預購食糧（含主食、補給）
- `行程/餐飲` - 行程中外食、行前聚餐
- `行程/飲品` - 行程中的飲料、果汁
- `行程/點心` - 登山、旅遊中的零食、小點
- `行程/裝備` - 購買登山器材
...
**禮物類別**：
- `禮物/家庭` - 買給家人的驚喜、禮物
- `禮物/外部贈送` - 送朋友、同事的禮物或禮金

**交通類別**：
- `交通/車資` - 他人代勞（計程車、Uber、大眾運輸、機票）
- `交通/行車` - 自駕變動成本（油錢、停車、過路費）
- `交通/維護` - 養車固定成本（稅金、保險、保養、維修）
- `交通/罰鍰` - 交通罰單
...
**判斷邏輯**（務必嚴格遵守）：
- **本質分類原則**：消耗品、稅務費用、公共事業、折扣、交通應精確分流：
    - 身體護理（護唇膏、防曬、洗沐）→ **`家庭/身體護理`**
    - 居家清潔（衛生紙、清潔劑）→ **`家庭/居家清潔`**
    - 公用事業（水費、電費、瓦斯、管理費）→ **`家庭/公用事業`**
    - 養車稅規費（牌照稅、燃料費、保險）→ **`交通/維護`**
    - 個人/綜合所得稅（所得稅、扣繳）→ **`其他/稅費支出`**
    - 個人車資（個人 Uber、我的公車費、共乘費）→ **`個人/車資`**
    - 家庭/共用交通（全家搭車、悠遊卡加值）→ **`交通/車資`**
- **退款與折扣**：
    - 通用折價、回饋、回饋金、折扣券 → **`折扣/優惠`**
    - 購物退稅 → **`折扣/退稅`**
- **退款對沖邏輯**：當金額為**負數**且品項包含「退款、退費、取消」時，應優先歸類於該項目的**原始類別**（例如：山屋退費歸入 `行程/住宿`）。
- **行程餐飲自動化**：只要「專案」不是「日常」，餐飲消費必須一律分類為 **`行程/餐飲`**。
- **無法精確匹配** → 選擇最接近的上層分類。

**預設規則**：**預設所有食品類消費為「家庭」類別**，只有在明確的個人情境下（如「公司聚餐」）才判定為「個人」類
"""
...
**重要**：僅當訊息中**明確包含**下表代墊相關關鍵字時，才判斷為代墊狀態。否則一律為「無」。
...
- 「**朋友**幫買票」→ 對象：`"朋友"`
- 「**同事**先墊餐費」→ 對象：`"同事"`
- 「**弟**代訂日本白馬房間10000元」→ 對象：`"弟"`
- 「**朋友**幫買演唱會門票3500元」→ 對象：`"朋友"`
- 「**同事**先墊演唱會門票3500元」→ 對象：`"同事"`
...
2. 包含「代」+對象+「買/付/墊」或「幫」+對象+「買/付/墊」→ **代墊**
   ...
   - **補充**：「幫+對象+代墊」也一律判為 **代墊**
3. 包含 對象+「代訂/代付/幫買/幫我買/先墊」→ **需支付**
   - **強制推斷**：若訊息以「對象+代訂/代付/幫買/幫我買/先墊」開頭，必須推斷對象並填入 `收款支付對象`
4. **完全沒有**以上關鍵字 → **無**（預設）

### 付款方式預設規則

- **代墊**項目：使用訊息中的付款方式（如「現金」、「刷卡」）
- **需支付**項目：若訊息未說明付款方式 → 預設 `"N/A"`（因他人代付，本人尚未支付）
  - 範例：「同事先墊午餐費150元」→ `payment_method="N/A"`, items: `{品項:"午餐費", 原幣金額:150, 代墊狀態:"需支付", 收款支付對象:"同事"}`
  - 範例：「弟代訂日本白馬房間10000元」→ `payment_method="N/A"`, items: `{品項:"日本白馬房間", 原幣金額:10000, 代墊狀態:"需支付", 收款支付對象:"弟"}`
- **不索取**項目：使用訊息中的付款方式

### 權威提示（必須遵守）

- 若訊息包含「已判定代墊狀態」與「收款支付對象」字樣，表示已解析完成 → **不得回傳缺少對象的錯誤**。
- 在上述情況下，若付款方式未提供，可填 `"N/A"`，**不得因付款方式為 N/A 回傳錯誤**。
- 若訊息包含「已解析金額」，**必須使用該金額**，不得回傳缺少金額錯誤。
- 若訊息**未**包含金額且無法解析金額，必須回傳錯誤，不得填入 `"N/A"`。
...
- 若訊息符合「對象+代訂/代付/幫買/幫我買/先墊」，收款支付對象即為該對象，不得回傳缺少對象錯誤
...
- 明確欄位名稱只支援：品項、分類、專案、原幣金額、金額、原幣別、幣別、匯率、付款方式、明細說明、明細、必要性、收款支付對象、代墊狀態。
- 不支援欄位同義詞（例如「費用」不視為金額）。
- 不得推測欄位或新值（僅在明確提供時才更新）。
- 一次只允許更新單一欄位，若同時指定多欄位 → 回傳 error。
- 付款方式欄位必須輸出標準名稱（請依對照表轉換），不得回 error。
- **金額規則**：允許負數金額（用於表示退款、折扣或回饋金）。
- 分類欄位必須輸出分類清單中的**完整路徑**（例如「教育/子女」），不可自建分類。
...
- 「上一筆分類改教育」→ `{{"intent": "update_last_entry", "fields_to_update": {{"分類": "教育/子女"}}}}`
- 「分類改 教育」→ `{{"intent": "update_last_entry", "fields_to_update": {{"分類": "教育/子女"}}}}`
- 「修改對象為妹妹」→ `{{"intent": "update_last_entry", "fields_to_update": {{"收款支付對象": "妹妹"}}}}`
- 「修改狀態為代墊」→ `{{"intent": "update_last_entry", "fields_to_update": {{"代墊狀態": "代墊"}}}}`
...
1. **判斷意圖**（僅支援以下 four 種 intent）：
   ...
   **注意**：不要使用開立其他 intent，只能使用上述四種！

2. **必填欄位強制規則（高優先）**：
   - **品項為必填**：若任何項目缺少品項，**必須回傳 error**，**不得補值**。
   - **禁止占位詞**：不得使用「品項# / 品項1 / 項目1 / 未指定 / 未知項目 / 未提供品項名稱 / N/A」等占位詞填入品項。
   - **純金額不是品項**：例如「120元」「$120」不可當作品項。
   - 多項目只要有一項缺品項 → 整筆回傳 error。
   - **缺付款方式或金額** → 除了「需支付」外，必須回傳 error，不得補值。
...
3. **單項目 vs 多項目判斷規則**：
   ...
   **識別步驟**：
   ...
   - **單項目範例**：
     ...
     - ✅ 「午餐 120 Line」→ 1個項目（午餐+120元），付款方式：Line Pay（無「元」符號）
     - ✅ 「大兒子英文課10堂 1800 Line」→ 1個項目（大兒子英文課10堂+1800元），付款方式：Line Pay（含數量）
     - ✅ 「請同事飲料 360 大戶」→ 1個項目（品項:「請同事飲料」），付款方式：大戶信用卡（此為一般支出，不是代墊）
   ...
   - **多項目範例**：
     ...
     - ✅ 「咖啡50；三明治35；Line」→ 2個項目（咖啡+50、三明治+35），付款方式：Line Pay

   - **付款方式關鍵字**（這些詞彙永遠是付款方式，不是品項）：
     - 關鍵字：狗卡、Line、現金、合庫、Richart、灰狗、大戶、綠卡、富邦、星展、華南紅、日圓現金
...
5. **記帳處理**：
   ...
   - **金額識別規則**：
     ...
     - **支援負數金額**：識別「-100」、「負50」等，用於表示退款、折價券、回饋金或退稅。
...
- 「用Line Pay，咖啡50元」（付款方式在前）
- 「同事先墊午餐費150元」（需支付：別人先付，付款方式預設 N/A）
- 「WSJ 4.99美元 大戶」（外幣）
...
請從圖片中抽取『消費明細』表格與卡片段落標題，輸出 JSON（不要多餘文字）。

重要規則：
- 表格欄位通常包含：消費日、入帳起息日、消費明細、新臺幣金額、外幣折算日、消費地、幣別、外幣金額。
- 同一張帳單可能包含多個卡片段落（例如：
  - Richart卡(原@GoGoicash御璽)… → 視為「台新狗卡」
  - Richart卡(原FlyGo鈦金商務)… → 視為「FlyGo 信用卡」
  )
  請為每筆明細填入 card_hint（例如 "台新狗卡" 或 "FlyGo 信用卡"），若無法判斷則填 null。
- 「國外交易服務費—93.00」這種行：
  - 破折號後的 93.00 是 reference_amount（指向對應消費的新臺幣金額），不是手續費本身。
  - 手續費本身金額請以該行的新臺幣金額欄位為準。

輸出格式：
{
  "status": "success" | "unclear" | "not_statement" | "incomplete",
  "message": "...",
  "lines": [
    {
      "card_hint": "台新狗卡" | "FlyGo 信用卡" | null,
      "trans_date": "YYYY-MM-DD" | null,
      "post_date": "YYYY-MM-DD" | null,
      "description": "...",
      "twd_amount": 1234.0,
      "fx_date": "YYYY-MM-DD" | null,
      "country": "TW"|"JP"|...|null,
      "currency": "TWD"|"JPY"|"USD"|"EUR"|null,
      "foreign_amount": 123.45 | null,
      "is_fee": true | false,
      "fee_reference_amount": 93.0 | null
    }
  ]
}

要求：
- 日期若是民國年（例如 114/08/17），請轉成西元（2025-08-17）。
- 金額請輸出數字（移除逗號）。
- 如果看起來不是台新信用卡帳單消費明細，回傳 status=not_statement。
- 如果太模糊無法辨識，回傳 status=unclear。
"""
...
1. **判斷圖片類型**：
   ...
   - **最高原則**：只要圖片中包含「品項/描述」與「金額數字」的對應關係，即便沒有標題，也請務必視為收據並嘗試提取。
   - **日文支援**：日文收據（含「領収書」、「レシート」字樣）是主要識別對象，請務必處理。
   - 若明顯非收據（如風景照、人物照、自然景觀），請回傳 "not_receipt"。
2. **提取資訊**（若為收據）：
   - **品項名稱**：購買的商品 or 服務名稱（例如：美式咖啡、三明治、拿鐵、午餐、便當等）。
     - 品項可能以各種格式出現：「咖啡」、「Coffee」、「美式咖啡（中杯）」等。
     - **完整性與多行合併要求**：
       - 請務必列出收據上的**每一筆**品項。即便收據較長，也請逐一提取所有明細，不要遺漏任何中間項目。
       - **重要**：若品項描述跨越多行（例如：第一行是品名，第二行是備註如「卵あり」），請將這些文字**合併**為一個意義完整的繁體中文名稱。
     - **翻譯規則（核心任務）**：你**必須**將所有非繁體中文的品項名稱（如日文、英文）**翻譯為自然、精確的繁體中文**。
       - **禁止**在「品項」欄位保留原始日文或英文。
       - 品牌名或特定產品請提供合適的中文譯名（例如：將「おにぎり」譯為「飯糰」；將「7PG金の食パン」譯為「7PG金的吐司」）。
     - **原文欄位**：請將原始的完整品項名稱（未翻譯前的合併文字）保留在「原文」欄位。
     - **銀行繳費通知**：品項通常為「代扣 - XXX」格式（例如：「代扣 - 台水水費」、「代扣 - 中華電信」）。
       - 提取時請**移除「代扣 - 」前綴**，只保留實際服務名稱（例如：「台水水費」、「中華電信」）。
       - 若品項包含繳費代號或帳號，可省略或放到明細說明中。
   - **金額**：每個品項的價格（可能標示為：50、$50、50元、NT$50、NTD 50、¥1000、1000円 等）。
     - **價格行解析（極重要）**：針對包含單價與數量的價格行（如：「@ 1,050 2點 2,100」），你**必須提取最右側的「總額」**（即 2,100）作為該品項的「金額」。絕對不要誤取中間的單價。
     - 金額通常在品項旁邊或下方。
     - 注意區分「單價」和「總計」。
     - **金額為純數字**：在 JSON 中的金額請只填寫純數字，不要包含貨幣符號或逗號。
   - **總金額**：所有品項加總（通常標示為：合計、總計、Total、應付金額等）。
   - **日期**：交易日期（若有，支援格式：2025/10/13、2025-10-13、114/10-13 等）。
     - **銀行繳費通知**：日期通常在開單 or 入帳位置，格式為「YYYY/MM/DD」。
   - **時間**：交易時間（若有，支援格式：17:22、14:30:05 等）。
   - **付款方式**：若圖片上有標示（如「現金」、「信用卡」、「行動支付」）。
     - **銀行繳費通知**：若無明確標示，可推測為轉帳或扣款。

3. **幣別與語言判斷**：
   - 支援 TWD、USD、EUR...等。
   - **日文收據**：品項為日文時，幣別通常為 JPY。
   - 台灣收據常用 $ 符號代表台幣，不是美金。
   - 若無法從圖片判斷幣別，預設為 TWD。
4. **品質檢查（寬容模式）**：
   - 只要能看清文字與數字，即便圖片角度歪斜或光線不足，也請努力進行辨識並回傳 "success"。
   - 只有在圖片完全模糊或完全沒有品項/金額資訊時，才回傳 "unclear" 或 "incomplete"。
   - **重要**：日文與英文收據是正確的記帳憑證，**不應**被判定為 "not_receipt"。
...
**注意**：
- `currency` 為整體幣別（可為 TWD/USD/EUR/JPY），作為 items 的預設值。
- `date` 格式為 YYYY-MM-DD（整體日期，作為 fallback）。
- `time` 格式為 HH:MM:SS 或 HH:MM（整體時間，作為 fallback）。
- `items` 為陣列，每個項目包含「品項」、「原文」、「金額」、「幣別」、「日期」、「時間」、「明細說明」。
- `品項` 為翻譯後的繁體中文名稱。
- `原文` 為收據上的原始品項名稱（若收據原本就是繁體中文，則「品項」與「原文」相同）。
- `日期` 格式為 YYYY-MM-DD，每個項目的獨立日期（若圖片上某項目無日期資訊，可使用最外層的 `date` 或設為 null）。
- `時間` 格式為 HH:MM:SS 或 HH:MM，每個項目的獨立時間（若圖片上某項目無時間資訊，可使用最外層的 `time` 或設為 null）。
- `明細說明` 為額外資訊，如數量、單價、商家分店名或繳費代號等（若無則為空字串）。
- `total` 為所有項目的總和。
...
合計         $80
```
→ 品項：美式咖啡(中)、三明治 | 原文：美式咖啡(中)、三明治 | 金額：45、35 | 總計：80
...
合計         270円
```
→ 品項：飯糰、茶 | 原文：おにぎり、お茶 | 金額：150、120 | 總計：270
...
應付金額              145
```
→ 品項：拿鐵咖啡、起司蛋糕 | 原文：拿鐵咖啡、起司蛋糕 | 金額：65、80 | 總計：145

**範例 4：簡易收據（只有總額）**
```
早餐  $50
```
→ 品項：早餐 | 原文：早餐 | 金額：50 | 總計：50
...
{{
  ...
  "date": "2025-10-28",
  "items": [
    {{"品項": "台水水費", "原文": "台水水費", "金額": 262, "分類": "家庭/公用事業", "日期": "2025-10-13"}},
    {{"品項": "中華電信", "原文": "中華電信", "金額": 199, "分類": "家庭/通訊/網路費", "日期": "2025-10-28"}},
    {{"品項": "中華電信", "原文": "中華電信", "金額": 599, "分類": "家庭/通訊/手機費", "日期": "2025-10-28"}}
  ],
  "total": 1060
}}
```
...
2. **品項和金額的辨識**：
   - **品項**：任何描述購買商品/服務的文字都算（咖啡、午餐、便當、拿鐵等）。
     ...
     - 若為外語品項，請將「品項」翻譯為自然、流暢的繁體中文，並將原始文字保留於「原文」欄位。
   - **金額**：任何數字+貨幣符號的組合（50、$50、50元、NT$50、NTD 50、¥、円 等）。
   - 只要能找到**至少一個品項**和**對應的金額**，就應該回傳 success。
   - **特別注意**：銀行繳費通知中的品項和金額可能較分散，但只要能對應即可。
3. **不要過度嚴格**：即便圖片略模糊、排版不整齊，只要能辨識出基本資訊就應該成功。
4. **品項要具體**：盡量保留原始品項名稱（如「美式咖啡(中)」而非「飲料」）。
5. **金額為數字**：在 JSON 中的金額不要包含貨幣符號，只保留數字。

**範例 6：複雜日本收據（多行品項與價格行）**
```
牛めし
卵あり
@ 1,050 2點 2,100
