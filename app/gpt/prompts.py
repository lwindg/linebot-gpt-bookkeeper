...
**行程餐飲自動化（高優先）**：
- **關鍵規則**：只要「專案」**不是**「日常」（例如是：日本玩雪、登山行程、特定旅遊），所有的餐飲消費（早餐、午餐、晚餐、咖啡、拉麵等）必須一律分類為 **`行程/餐飲`**。
- **理由**：這是為了將「旅遊開銷」與「日常生活開銷」在圖表統計上徹底分開。

**消耗品本質回歸（高優先）**：
- **關鍵規則**：針對「衛生/護理消耗品」，分類必須依性質精準分流：
    - 身體護理（護唇膏、防曬、洗沐）→ **`家庭/身體護理`**
    - 居家清潔（衛生紙、清潔劑）→ **`家庭/居家清潔`**
    - 公用事業（水費、電費、瓦斯、大樓管理費）→ **`家庭/公用事業`**
    - 養車稅規費（牌照稅、燃料費、保險）→ **`交通/維護`**
    - 個人/綜合所得稅（所得稅、扣繳）→ **`稅費支出`**
- **追蹤方式**：僅透過 **`專案`** 欄位標記行程（如：日本玩雪）。這能確保分類數據的純粹性。
...
5. **記帳處理**：
   ...
   - **品項提取規則**（品項 = 實際購買的物品 或 代墊、他人代墊的行為）：
   ...
   - **分類與退款對沖規則**：
     ...
     - **折扣與回饋**：通用折扣歸入 `折扣/優惠`，購物退稅歸入 `折扣/退稅`。
...
**注意**：
- `currency` 為整體幣別（可為 TWD/USD/EUR/JPY/GBP/AUD/CAD/CNY），作為 items 的預設值。
