...
**判斷邏輯**（務必嚴格遵守）：
- **本質分類原則**：消耗品、稅務費用、公共事業、折扣應精確分流：
    - 身體護理（護唇膏、防曬、洗沐）→ **`家庭/身體護理`**
    - 居家清潔（衛生紙、濕紙巾、洗手乳、清潔劑）→ **`家庭/居家清潔`**
...
- 若訊息**未**包含金額且無法解析金額，必須回傳錯誤，不得填入 `"NA"`。
...
5. **記帳處理**：
   ...
   - **分類與退款對沖規則**：
     ...
     - **退款與折扣**：
         - 通用折價、回饋、回饋金、折扣券 → **`折扣/優惠`**
         - 購物退稅 → **`折扣/退稅`**
     - **退款對沖邏輯**：當金額為**負數**且品項包含「退款、退費、取消」時，應優先歸類於該項目的**原始類別**（例如：山屋退費歸入 `行程/住宿`），而非雜項。這能確保 Notion 統計時呈現正確的「淨支出」。
